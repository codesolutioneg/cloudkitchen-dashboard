...
function RolesPage() {
  ...
  return (
    <>
      <PageHeader
        title="Roles & Permissions" description="Define what dashboard users can access."
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            <div><label className="mb-1 block text-sm font-semibold">Name</label><input value={name} onChange={(e) => setName(e.target.value)} className="h-10 w-full rounded-[10px] border border-border bg-card px-3 text-sm" placeholder="e.g. Kitchen Manager" /></div>
