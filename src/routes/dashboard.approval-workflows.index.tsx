      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent><DialogHeader><DialogTitle>Create approval workflow</DialogTitle></DialogHeader>
          <div className="space-y-2">
            ...
            <input placeholder={t("Entity type")} value={form.entityType} onChange={(e) => setForm({ ...form, entityType: e.target.value })} className="h-10 w-full rounded-md border border-border bg-card px-3 text-sm" />
          </div>
  ...
  const cols: Column<ApprovalRequest>[] = [
    ...
    { key: "actions", header: "", cell: (r) => (
      <div className="flex justify-end gap-1">
        <button onClick={() => decide(r.id, "approved")} className="rounded-md bg-success px-2.5 py-1 text-xs font-semibold text-white hover:opacity-90">{t("Approve")}</button>
        <button onClick={() => decide(r.id, "rejected")} className="rounded-md bg-destructive px-2.5 py-1 text-xs font-semibold text-destructive-foreground hover:opacity-90">{t("Reject")}</button>
      </div>
