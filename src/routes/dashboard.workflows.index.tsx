...
function WorkflowsPage() {
  return (
    <>
      <PageHeader title={t("Workflows")} description={t("Design order workflows, steps, transitions and monitor instances.")} />
      <Tabs defaultValue="templates" className="w-full">
        <TabsList className="mb-4">
          <TabsTrigger value="templates">{t("Templates")}</TabsTrigger>
          <TabsTrigger value="instances">{t("Instances")}</TabsTrigger>
          <TabsTrigger value="reference">{t("Standard Order Flow")}</TabsTrigger>
        </TabsList>
  ...
  return (
    <>
      ...
      <DataTable columns={cols} rows={data} loading={isLoading}
        ...
        emptyTitle={t("No workflow templates")} />
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent><DialogHeader><DialogTitle>{t("Create workflow")}</DialogTitle></DialogHeader>
...
function InstancesTab() {
  ...
  return <DataTable columns={cols} rows={data} loading={isLoading} emptyTitle={t("No workflow instances")} />;
}
function ReferenceFlow() {
  return (
    <div className="card-elevated space-y-6 p-6">
      <div>
        <p className="mb-3 text-sm font-semibold text-foreground">{t("Delivery order lifecycle:")}</p>
      ...
      <div>
        <p className="mb-3 text-sm font-semibold text-foreground">{t("Pickup path:")}</p>
        <div className="flex flex-wrap items-center gap-2">
